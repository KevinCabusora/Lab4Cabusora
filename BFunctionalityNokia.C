...
drawBlock:
	...
	mov		#1, R12             // replaced code that did 8 columns of 8 pixels with 
	mov		#0x18, R13          // code that would make an approximate ball shape
	call	#writeNokiaByte
	mov		#0x3C, R13
	call	#writeNokiaByte
	mov		#0x7E, R13
...
delayLoop:
	...
	nop                     // Creates a reasonable delay time so process is not running too fast
	nop
	nop
	nop
	nop
